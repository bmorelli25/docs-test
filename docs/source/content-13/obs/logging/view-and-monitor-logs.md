...
## Required PRODUCT_NAME privileges

Viewing data in Logs Explorer requires `read` privileges for **Discover** and **Integrations**.
For more on assigning Kibana privileges, refer to the [(PRODUCT_NAME) privileges](http://example.co)/kibana-privileges.html) docs.
...
Refer to the [Discover](http://example.co)/discover.html) documentation for more information on updating the table.
...
From the log details of a document with ignored fields, as shown by the degraded document indicator PRODUCT_NAME, expand the **Quality issues** section to see the name and value of the fields that were ignored.
