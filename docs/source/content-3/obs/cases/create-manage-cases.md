...
When you export cases as [saved objects](http://example.co)/managing-saved-objects.html), the case files are not exported.
</DocCallOut>
...
Follow the steps in [Send alerts by email](http://example.co)/ec-watcher.html#ec-watcher-allowlist).
